...
#align(center)[
  ...
  #text(size: 14pt, weight: "bold")[Создание информационной системы для платформы обмена сновидениями "Dream Marketplace"]

  \

  #text(size: 12pt)[Курсовая работа\
...
#align(center)[
  #text(size: 12pt, weight: "bold")[1 этап]
]
...
Полученные результаты оформляются как лоты, которые можно выставлять на продажу или хранить в коллекции.
...
+ *Маркетплейс*
    ...
    - Лоты могут находиться в двух состояниях: *«активный» *(доступен для покупки) или *«в коллекции»* (только для владельца).
...
#table(
  ...
  [Второстепенные актеры], [Система торговой площадки, модератор (опционально)],
...
#table(
  ...
  [Главный актер], [Пользователь],
  [Второстепенные актеры], [Система страниц лота, система прав/лицензий],
