...
В рамках второго этапа курсовой работы на основе ранее созданных ER- и даталогической моделей была реализована реляционная база данных в СУБД PostgreSQL. В процессе реализации, включавшей написание функций и создание индексов, даталогическая модель была доработана: в нее добавили отсутствующие поля и ограничения. В результате была получена отлаженная структура БД, отвечающая поставленным требованиям и обеспечивающая целостность хранимой информации. Разработанная система обладает потенциалом для дальнейшей модернизации в случае появления новых требований.