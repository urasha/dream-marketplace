...
*Dream Marketplace* — это онлайн-платформа для фиксации, визуализации и обмена сновидениями. Пользователи создают записи своих снов в текстовом формате, после чего могут превратить их в мультимедийный контент (изображения или видео) с помощью встроенного модуля интеграции с сервисом *GenAPI*.

*GenAPI* предоставляет REST API для генерации изображений и видео на основе текста. Наш встроенный AI-сервис будет взаимодействать с ним через HTTP-запросы, автоматически получая готовые материалы и сохраняя их в системе. Для пользователя это выглядит как встроенная функция платформы и не требует прямого обращения к сторонним сервисам.
...
+ *Фиксация снов*
    ...
    - Текстовая запись сна используется как основа для дальнейшей визуализации и формирования мультимедийного контента.

+ *Визуализация*
    - Наш AI-сервис обращается к GenAPI и преобразует текстовую запись сна в мультимедийный объект (изображение или короткое видео).  
...
*FR1.* Система должна предоставлять пользователю возможность входа через внешнего поставщика удостоверений (Яндекс ID).

*FR2.* При первом успешном входе система должна автоматически создать локальный профиль пользователя; отдельная регистрация не требуется.

*FR3.* Система не должна запрашивать и хранить пользовательские пароли.

*FR4.* Система должна однозначно идентифицировать пользователя по стабильному идентификатору, предоставляемому Яндекс ID.

*FR5.* Система должна сохранять минимально необходимую информацию профиля, полученную от Яндекс ID, для работы функционала платформы.

*FR6.* Система должна корректно обрабатывать отказ пользователя в предоставлении доступа и недоступность Яндекс ID, информируя пользователя и не создавая неполных профилей.

*FR7*. Система должна предоставлять пользователю возможность просматривать и редактировать свой профиль.

*FR8*. Система должна предоставлять пользователю возможность создавать текстовую запись сна с указанием заголовка, описания, категории и тегов.

*FR9*. Система должна предоставлять пользователю возможность указывать для записи уровень приватности (приватная / публичная).  

*FR10*. Система должна предоставлять пользователю возможность просматривать список своих записей. 

*FR11*. Система должна предоставлять пользователю возможность запрашивать генерацию визуализации (изображения/видео) для выбранной записи сна (отправлять задачу в очередь).  

*FR12*. Система должна предоставлять пользователю возможность просматривать сгенерированные визуализации, выбирать и сохранять понравившийся вариант.  

*FR13*. Система должна обеспечивать пользователю возможность оформлять визуализацию как лот: задавать название, описание, цену.  

*FR14*. Система должна предоставлять пользователю возможность просматривать каталог публичных лотов и визуализаций с фильтрацией по категориям, тегам, автору и сортировкой по дате.  

*FR15*. Система должна предоставлять пользователю возможность просматривать страницу лота с превью, описанием и т.д.  

*FR16*. Система должна обеспечивать пользователю возможность покупать лот через платёжную систему и закреплять право на владение за покупателем при успешной оплате.

*FR17*. Система должна предоставлять пользователю возможность оставлять комментарии и ставить рейтинг лоту.  

*FR18*. Система должна предоставлять пользователю возможность пополнения внутреннего баланса через эмуляцию платёжной системы, включая выбор суммы, подтверждение операции и зачисление средств.

*FR19*. Система должна предоставлять пользователю возможность просматривать свой баланс и покупки. 

*FR20*. Система должна предоставлять пользователю возможность запросить повторную генерацию визуализации (получить альтернативный вариант). 

*FR21*. Система должна обеспечивать пользователю возможность скачивания купленной визуализации в поддерживаемых форматах, включая графические (JPEG) и видео (MP4) файлы.

#v(0.25cm)

*Требования администратора:*

*FR22*. Система должна предоставлять администратору возможность просматривать и модерировать лоты (блокировать неприемлемые лоты).  
...
*U1*. Система должна предоставлять адаптивный пользовательский интерфейс, корректно отображающийся на:

- *Десктопах*: с разрешением от 1366×768 до 1920×1080.

- *Планшетах*: с разрешением от 768×1024  и 1024×768.

- *Смартфонах*: с разрешением от 360×640 до 414×896.
...
*REL2*. Система должна обеспечивать надежную обработку пользовательских запросов без задержек.

#v(0.25cm)

*Требования к производительности*

*PER1*. Система должна обеспечивать быстрый отклик при чтении данных: время ответа не более 1 секунды для большинства (95%) запросов в условиях нормальной нагрузки.

*PER2*. Генерация визуализаций должна выполняться в фоновом режиме, чтобы пользовательский интерфейс оставался доступным и отзывчивым. Время ожидания результата пользователем не должно превышать 2 минут.

*PER3*. Время первого отображения основной страницы при стандартном подключении (20 Мбит/с) не должно превышать 3 секунд.

*PER4*. Система должна обеспечивать стабильную работу при одновременной активности до 80 пользователей.

#v(0.25cm)

*Требования к поддерживаемости*  

*SUP1*. Система должна генерировать структурированные логи ошибок и событий в формате, удобном для агрегирования и анализа.  
...
*A3*. Система должна управлять конфигурацией через переменные окружения; "секреты" не должны храниться в кодовой базе.  

*A4*. Система должна взаимодействовать с API *Yandex Object Storage* для загрузки, хранения и получения мультимедийного контента.

*A5*. Система должна обеспечивать аутентификацию пользователей через *Яндекс ID* (OAuth 2.0).
...
#precedent-table(
  [Прецедент], [Вход через Яндекс ID (с автосозданием профиля)],
  [ID], [1],
  [Краткое описание], [Делегированная аутентификация через Яндекс ID. При первом успешном входе создаётся локальная запись пользователя],
  [Главный актёр], [Гость],
  [Второстепенные актёры], [Поставщик удостоверений (Яндекс ID)],
  [Предусловия], [Пользователь не авторизован; сервис Яндекс ID доступен],
  [Основной поток], [
    1. Гость выбирает «Войти через Яндекс».
    2. Система инициирует перенаправляет на Яндекс ID.
    3. Пользователь проходит аутентификацию у Яндекс ID.
    4. Система получает ответ, валидирует его, получает токены.
    5. Система находит локального пользователя по или создаёт новую запись.
    6. Создаётся серверная сессия.
    7. Пользователь перенаправляется в приложение.
  ],
  [Альтернативный поток], [
    Отмена/ошибка у Яндекс ID → информационное сообщение, остаёмся на экране входа.
  ],
  [Постусловия], [Пользователь авторизован; при первом входе создана локальная учётная запись]
)
...
#precedent-table(
  ...
  [Основной поток], [
    ...
    4. Система сохраняет запись в базу данных.
  ],
...
#precedent-table(
  ...
  [Второстепенные актеры], [Сервис оплаты, система транзакций, продавец],
  ...
  [Основной поток], [
    ...
    3. Сервис оплаты обрабатывает оплату и возвращает результат.
...
Фронтенд будет реализован на *Vue.js* с использованием SPA-подхода (Single Page Application). 
...
Основные модули бэкенда:
    - *Модуль авторизации:* вход через Яндекс ID, автосоздание локального профиля и выпуск/валидация JWT для доступа к API.
    - *Модуль пользователей:* хранение и редактирование профиля и ролей.
